...
#
# Output:
#   ~/printer_data/config/input_shaper/belt-tension-resonances_YYYY-MM-DD-HHMMSS.png
######################################################################
...
echo "Using lower CSV: ${NEWLOWER}"
echo "Using upper CSV: ${NEWUPPER}"
echo "Output: ${OUTPUT}"

"${GRAPH_ACCELEROMETER}" -c "${NEWLOWER}" "${NEWUPPER}" -o "${OUTPUT}"
